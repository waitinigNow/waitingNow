                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: () async {
                            ...
                            // 배정 해제 실행
                            String value = await deskController.deskOut(widget.deskVO.deskStoreNumber);
    ...
    void toggleCheckbox(bool? value) {
      ...
      setState(() {
        if (!tableAvailable) {
          ...
          if (isChecked) {
            deskGet.checkedDesks.value.add(widget.deskVO.deskStoreNumber); // 체크 해제 시 추가함
            containerColor = Color(0xFFFFEBCD); // 체크되었을 때 배경 색상 변경
          } else {
            deskGet.checkedDesks.value.remove(widget.deskVO.deskStoreNumber); // 체크 해제 시 추가함
            ...
            Row(
              ...
              children: [
                FlutterSwitch(
                  width: 95,
                  height: 40,
                  disabled: tableAvailable, // 배정 가능하면 disabled함
                  showOnOff: true,
                  valueFontSize: 14.0,
                  ...
                  onToggle: (val) {
                    alert("배정 해제", "${widget.deskVO.deskStoreNumber}번 테이블을 배정 해제 하시겠습니까?");
                  },
                ),
                SizedBox(width: 30)
              ],
