...
public class DeskService {
    ...
    public SentDeskAssignVO assignDesk(SentDeskAssignVO sentdeskAssignVO) throws Exception {
        ...
        // preorderExist 값 채우기
        if (preorderDAO.selectByWaiting(sentdeskAssignVO.getWaitingNumber()).isEmpty())
        ...
        waiting.setWaitingNumber(deskAssignVO.getWaitingNumber());
        waiting.setWaitingAvailable(0);
        waiting.setMemberNumber(deskAssignVO.getMemberNumber());
    ...
    public void assignOutDesk(DeskAssignVO deskAssignVO) throws Exception {
        ...
        if (deskDAO.deskSitAvailable(deskAssignVO) != 1) {
            throw new IllegalStateException("배정되어 있지 않은 " + deskAssignVO.getDeskStoreNumber() + "번 식탁을 퇴장 처리 하는군.");
        }
